...
class CalendarMain extends React.Component {
  ...
  printCalendarWeeks() {
    ...
    return this.state.weeks.map((week, i) => {
      if (i === 0) return <CalendarWeek key={i} week={week} initialWeek={true} lastWeek={false} />
      if (i === numberOfWeeks - 1) return <CalendarWeek key={i} week={week} initialWeek={false} lastWeek={true} />

      return <CalendarWeek key={i} week={week} initialWeek={false} lastWeek={false} />
    });
  }
